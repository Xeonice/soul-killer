...
用户选择「创建」后 SHALL 交互式收集世界名称，然后进入 WorldCreateCommand 向导收集 display_name 和 description。

#### Scenario: 交互式创建新世界
- **WHEN** 用户在菜单选择「创建」，输入世界名称 "night-city"
- **THEN** 进入向导收集 display_name 和 description 后创建世界目录
