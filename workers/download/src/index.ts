 ...
 *   GET /examples/catalog.json     → skill catalog (R2 examples/catalog.json, JSON)
 ...
 *   examples/catalog.json           — skill catalog (generated by scripts/build-catalog.ts)
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // GET /examples/catalog.json — skill catalog (JSON, CORS-enabled)
    if (path === '/examples/catalog.json') {
      const r2Obj = await env.RELEASES.get('examples/catalog.json')
      if (!r2Obj) return new Response('Catalog not yet published', { status: 404 })
      return new Response(r2Obj.body, {
        headers: {
