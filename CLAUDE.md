...
- `state/` — Runtime state management CLI. This directory is the single source of truth for **both sides** of the state lifecycle: Soulkiller's vitest suite imports these modules directly for unit testing, and the packager copies them byte-for-byte into every exported skill's `runtime/lib/` + `runtime/bin/` directories. All `.ts` files only use bun stdlib (zero npm dependencies) so they can be shipped into skills without a `node_modules` step. Contains: `mini-yaml.ts` (flat yaml parser for state.yaml/meta.yaml), `schema.ts` (StateSchema types + applyDelta), `script.ts` (script.json loader), `io.ts` (atomic file I/O + SaveType/SavePaths types), `init.ts` / `apply.ts` / `validate.ts` / `rebuild.ts` / `reset.ts` / `save.ts` / `list.ts` (state CLI subcommands — 8 total), `main.ts` (CLI dispatcher), `doctor.sh` (POSIX shell health check), `state.sh` (bash wrapper packaged as `runtime/bin/state`).
- `lint/` — Author-side template lint runs inside the soulkiller process at export time. Catches yaml syntax bugs, schema-key naming violations, axis cross-reference mismatches between story-spec axes and SKILL.md schema examples, and (since skill-runtime-bun-state) runtime CLI consistency rules (`PHASE_0_DOCTOR_PRESENT`, `STATE_APPLY_PRESENT`, `NO_EDIT_STATE_YAML`). Failures log to stderr but never block export — by design, the author audits the report manually.
...
- **Prose style anchor** (prose-style-anchor change): every new export is required to call `set_prose_style` (6 步工作流中的 Step 3) before any `add_character`. The tool locks in a story-level `ProseStyle` (target_language / voice_anchor / forbidden_patterns / ip_specific / optional character_voice_summary) that downstream Phase 1/2 LLMs must obey as a hard constraint to eliminate translatese from all generated Chinese prose. The universal anti-translatese pattern library lives at `src/export/prose-style/zh-translatese-patterns.ts` and is inlined into the `set_prose_style` tool description so the export agent decides with concrete symptomology in context. `ExportBuilder.build()` throws if prose_style is missing — the fallback branch in templates only exists for legacy archives loaded on the player side.
- **Phase 1 full-read enforcement** (phase1-full-read-enforcement change): Phase 1 in SKILL.md now enforces full-file reads to fix a diagnosed bug where LLMs default to `Read(lines 1-50)` partial pagination, missing most of the source material. The packager injects **`expectedFileCount` / `expectedTextSizeKb`** budget anchors (computed from `countMdFilesInMap` / `estimateMdTextSizeKb` on the in-memory archive map) into a "上下文预算与全量读取" declaration at the top of Phase 1, explicitly telling the LLM "约 N 个文件 / 约 M KB / 1M token context / 不得使用 offset 或 limit 参数 / 不要防御性地节省". Phase 1 also starts with a **re-Read story-spec.md** step to fix Phase 0 → Phase 1 context pollution (Phase 0 only reads the first 50 lines for `acts_options`, missing the prose_style / Story State sections further down). Creation steps are now numbered **Step 0 - Step 7** (8 steps), where **Step 0 is a mandatory data-loading report** — LLM must output a structured markdown table listing every file it read + line count before Step 1 is allowed. The final self-check layer is **Step 5.h (multi-char) / Step 5.e (single-char) — 数据覆盖完整性**: validates line counts against typical thresholds (identity.md > 50, style.md > 40, behaviors/*.md > 20) and triggers a re-Read when a file is suspiciously short.
- **Phase 2 LLM trained-default behavior enforcement** (same change, scope extension): Phase 2 in SKILL.md also fights a family of "chatbot-default" LLM behaviors that leak into visual-novel narration. The 禁止事项 section in both engines is restructured into explicit categories (剧情结构 / 控制流自暂停 / 进度存档暴露 / 聊天机器人式元叙述 / 选项标签污染). 场景流转规则 now explicitly enumerates "你只在 4 种情况下停止渲染" (scene end + AskUserQuestion / 💾 save flow / free-text reply + AskUserQuestion / ending) and declares apply_consequences → render-next-scene as a single atomic action. Every AskUserQuestion **must** append a "💾 保存当前进度" option after the script's choices — this is the only allowed non-script option. Continuous multi-scene rendering is normal behavior as long as each scene ends with an AskUserQuestion of the literal copy of `choices[i].text` + 💾 from the script.
...
- Runtime persistence layout inside the skill archive:
  ...
  - `runtime/bin/state` — bash wrapper, the LLM-facing entrypoint. Hides bun behind `bash runtime/bin/state <subcommand>`.
  - `runtime/bin/doctor.sh` — pure POSIX sh health check. Detects platform, checks `$HOME/.soulkiller-runtime/bin/bun`, emits structured stdout for LLM parsing.
  - `runtime/lib/*.ts` — bun TypeScript implementation layer. Zero npm dependencies (uses only bun stdlib + inline mini-yaml parser). LLM never reads or references these files directly.
- Phase -1 runs **six-fold** validation via `bash runtime/bin/state validate <script-id> [<save-type>]` which returns structured JSON: (1) `DANGLING_SCRIPT_REF`, (2) `STATE_SCHEMA_MISSING`, (3) `INITIAL_STATE_MISMATCH`, (4) `CONSEQUENCES_UNKNOWN_KEY`, (5) `SHARED_AXES_INCOMPLETE`, (6) `FLAGS_SET_MISMATCH`. Continue-game (`--continue` flag) adds a 7th check on state.yaml field-set alignment (`FIELD_MISSING` / `FIELD_EXTRA` / `FIELD_TYPE_MISMATCH`) and the repair menu dispatches to `state rebuild` or `state reset` — LLM never hand-patches the yaml.
- "从头再来" dispatches to `bash runtime/bin/state reset <script-id>`, which reloads `script.initial_state` atomically. "生成新剧本" in Phase -1 is the only way to create a fresh story.
...
- **skill-runtime-bun-state change** (this change): Phase 2 state writes moved from LLM `Edit` calls to a bun CLI shipped inside the archive. Errors B (clamp miscalculation), C (missed consequences key), D (wrong type), E (state/meta desync) are now **structurally impossible** because consequences math happens in TypeScript, not LLM prose. Error F (LLM skips `state apply` entirely) is still possible but falls outside this change's scope. Skill format constraint narrowed from "any Claude Code user" to "any Unix-like Claude Code user" (macOS / Linux / WSL; Windows native shell explicitly rejected). Bun runtime is a one-time per-machine bootstrap installed to `$HOME/.soulkiller-runtime/` (isolated from user's system bun, no PATH pollution). script format changed from YAML to JSON because runtime/lib cannot ship a YAML parser without a node_modules dependency — `JSON.parse` is built into bun. state.yaml and meta.yaml stay as yaml (human-readable save files, parsed by the ~40-line `mini-yaml.ts`).
...
- **Unit tests** (`tests/unit/`) — Pure logic: adapters, parsers, config, glitch engine
  ...
  - **Packager runtime tests** (`tests/unit/export-packager-runtime.test.ts`) — Verify `injectRuntimeFiles` copies all state CLI files byte-for-byte, marks `runtime/bin/state` + `runtime/bin/doctor.sh` as executable in the zip archive, and that `countMdFilesInMap` / `estimateMdTextSizeKb` exclude `runtime/` from Phase 1 context budget.
