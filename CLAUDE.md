...
- **Phase 1 full-read enforcement** (phase1-full-read-enforcement change): Phase 1 in SKILL.md now enforces full-file reads to fix a diagnosed bug where LLMs default to `Read(lines 1-50)` partial pagination, missing most of the source material. The packager injects **`expectedFileCount` / `expectedTextSizeKb`** budget anchors (computed from `countMdFilesInMap` / `estimateMdTextSizeKb` on the in-memory archive map) into a "Context Budget and Full-Read Enforcement" declaration at the top of Phase 1, explicitly telling the LLM the file count, KB estimate, and that offset/limit parameters must not be used. Phase 1 also starts with a **re-Read story-spec.md** step to fix Phase 0 → Phase 1 context pollution. Creation steps are numbered **Step 0 - Step 7** (8 steps), where **Step 0 is a mandatory data-loading report**. The final self-check layer validates line counts against typical thresholds and triggers re-Read when a file is suspiciously short.
- **Phase 2 LLM trained-default behavior enforcement** (same change, scope extension): Phase 2 in SKILL.md also fights a family of "chatbot-default" LLM behaviors that leak into visual-novel narration. The prohibited actions section in both engines is restructured into explicit categories (plot structure / control flow self-pause / save progress exposure / chatbot-style meta-narration / option label pollution). Scene transition rules explicitly enumerate the 4 situations where rendering stops (scene end + AskUserQuestion / 💾 save flow / free-text reply + AskUserQuestion / ending) and declare apply_consequences → render-next-scene as a single atomic action. Every AskUserQuestion **must** append a "💾 Save current progress" option after the script's choices — this is the only allowed non-script option.
...
- "Start over" dispatches to `bash runtime/bin/state reset <script-id>`, which reloads `script.initial_state` atomically. "Generate new script" in Phase -1 is the only way to create a fresh story.
