...
- **Author skill version** (skill-author-version change): the export wizard's `entering-version` step (between output selection and agent start) collects the author-declared skill version and writes it to `soulkiller.json.version`. Three version fields coexist with distinct roles: `engine_version` (runtime contract), `soulkiller_version` (build metadata, from `process.env.SOULKILLER_VERSION`), and `version` (author's release number — the one users see in `skill list` and whose diff drives `skill update`). Default suggestion: first export → `0.1.0`; re-export → `bumpPatch(previous)`. `build-catalog.ts` reads `version` from each archive's soulkiller.json; archives lacking it get `0.0.0` + warning (reserved for legacy back-fill). Lint rule `AUTHOR_VERSION_PRESENT` warns on missing / `0.0.0` at export time; not blocking.
...
- **README skill catalog auto-generation** (skill-catalog-autogen change): The "可装列表" table inside `README.md` is regenerated from the live archives instead of maintained by hand. Each `.skill` archive's `soulkiller.json` now carries three display fields — `world_slug` (kebab-case short slug), `world_name` (human-readable name), `summary` (single-line blurb) — populated by the LLM in `set_story_metadata` and confirmed by the author in a post-agent `entering-catalog-info` wizard step (triggered by the `catalog_confirm_request` event from `finalize.ts`). `scripts/build-skill-catalog.ts` scans `examples/skills/*.skill`, reads each manifest (falling back to `SKILL.md` frontmatter `description` when summary is missing, and `skill_id` when slug is missing), and replaces the content between `<!-- SKILLS:START -->` / `<!-- SKILLS:END -->` markers in README. The `sync-readme` job in `release.yml` runs the script on every `v*` tag push and commits any diff back to `main`; the `verify-skill-catalog` job in `ci.yml` runs `--check` on every PR to block drift.
